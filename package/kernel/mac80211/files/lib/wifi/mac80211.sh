...
detect_mac80211() {
	...
	for _dev in /sys/class/ieee80211/*; do
		...
		uci -q batch <<-EOF
			...
			set wireless.default_radio${devidx}.ssid=${ssid}-$(cat /sys/class/ieee80211/${dev}/macaddress | awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)-$(head /dev/urandom |tr -cd a-f0-9|cut -c 1-6)
